...
2. 拍照件**转成可编辑 Word**，并且**保留原有排版**（文字放在按原位置摆的文本框里，仍可复制修改）。
3. **证件二合一**：身份证正反面、户口本两页拍照后拼到一张 A4，**必须按实物尺寸打**（缩了复印件会被退回重做）。见 `docs/10-证件二合一.md`。
4. **工作区**：工作区文件夹（默认 `C:\打印\待打印`，设置里可改）和微信收到的文件**自动出现在首页**，不用在文件夹里翻找。

**刻意不做「打印文档」这个入口**：原件是 Word/Excel/PDF 时直接在 Office 里改好再打就行，工具在这上面帮不了忙（还多一道转换）。需要打印的时候都是从具体的活儿进去 —— 照片变清楚 / 证件 / 工作区里的文件，最后统一走「打印预览」页（选打印机、份数、纸张，预览画的是整张纸）。
...
- **不要在 UI 线程跑 OCR、Office COM 转换、全分辨率图像处理、读大图、扫目录** —— 一律丢 `run_async` / 子进程。界面卡住的时候 Windows 画的是一张定格的旧画面，长辈看到的就是"按钮变形、点了没反应"。
- **`run_async` 的返回值可以不接，但回调别指望自己活着**：`workers.run_async` 已经自己留了强引用（`_RUNNING`），改那段代码时不要把它删了 —— 否则 Worker 被回收，回调静默丢失。
...
- 打印那条路只有黑白（店里那台机器如此）。「黑白 / 彩色」开关只影响**另存为**的图片和 PDF，旁边必须写明「本店打印机只有黑白」
...
src/shop_print/
  ...
  paths.py  所有路径（缓存、日志、工作区、保存位置、样式表、模型）的**默认值**统一在此定义，不要散落硬编码
            （工作区和保存位置可以在设置里改，实际用哪个问 config.workspace_dir / config.save_dir）
