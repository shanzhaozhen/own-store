...
4. 微信收到的文件**自动出现在首页**，不用在文件夹里翻找。

**刻意不做「打印文档」这个入口**：原件是 Word/Excel/PDF 时直接在 Office 里改好再打就行，工具在这上面帮不了忙（还多一道转换）。需要打印的时候都是从具体的活儿进去 —— 照片变清楚 / 证件 / 微信收到的文件，最后统一走「打印预览」页（选打印机、份数、纸张，预览画的是整张纸）。
