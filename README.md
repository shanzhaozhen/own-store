...
| 功能 | 解决什么 |
|---|---|
| **照片变清楚再打印** | 手机拍的文档带阴影、纸张发黄，黑白机直接打一片脏灰。自动去底增强，背景压白、文字提黑；认出整张 A4/A3 纸会裁正拉平（裁剪能关、边缘能调），可存成图片或 PDF（黑白 / 彩色都行，彩色走白平衡尽量还原原色） |
| **照片转成文字文档** | OCR + 版面重建，输出**保留原有排版**的 Word：文字放在按原位置摆的文本框里，能选中、复制、改错字 |
| **证件印一张纸** | 身份证正反面、户口本两页拼到一张 A4，**按实物尺寸打**（85.6×54mm），尺寸不保真就明确说出来；黑白 / 彩色可选（红色国徽、蓝章能留住） |
| **工作区** | 监控工作区文件夹（默认 `C:\打印\待打印`，设置里可改）和微信接收目录；粘贴、拖进来的文件也先落到这里，再由长辈点「打印 / 变清楚 / 拼证件 / 转文字」进对应功能 |
...
- **裁的时候往外放一圈**，白边算进声明尺寸里：证件有一圈边、四角是圆的，贴着边裁就切掉了；而把白边一起当成卡片会小印 3.5%（实测 85.6mm 印出来 82.6mm）
- **不保真就说出来**：真放不下才缩，并且在界面上写明"已缩小到 93%"，不悄悄缩
- **深浅留一个滑块**：拍照效果差别太大（逆光、暖光、旧证件），身份证满版防伪底纹一浓就把字埋了 —— 滑块只重画不重新抠图，拖一下 0.3 秒出结果

尺寸这条线由断言守着（`tests/test_physical_size.py`）：每个预设的横放竖放各跑一遍，打开生成的 PDF 直接量毫米；再从合成照片走完整条链路，把纸渲染成 300dpi 位图**量有墨的范围**（这一条才拦得住"白边挤掉卡片"）。顺带把整条打印链路的产物都量了一遍 —— 图片→PDF、txt→PDF、OCR→Word、证件→PDF 全部 210×297mm。**这一遍对出一个一直存在的问题：OCR 转出来的 Word 原来是美国 Letter（215.9×279.4mm）**，python-docx 自带模板就是 Letter，已显式改成 A4。

细节和预设表见 [docs/10-证件二合一](docs/10-证件二合一.md)。

<p align="center">
  <img src="docs/images/工作区.png" width="720" alt="工作区：一个文件一张大卡片">
</p>
...
src/shop_print/
  core/     纯逻辑，不 import 任何 Qt —— 第二阶段的服务端 / 店铺 Agent 要原样复用
    ...
    intake.py     文件从哪来：微信目录 / 工作区目录 / 剪贴板 / 拖拽
...
装到店铺机：双击 `scripts\安装到店铺电脑.bat` —— 拷到 `C:\ShopPrint\`（旧版本自动改名留着好回滚）、公共桌面放「打印助手」和「待打印」（工作区文件夹）两个图标、可选开机自启。
