...
一屏五张大卡片，没有菜单栏、没有工具栏、没有设置按钮；正文 ≥16pt、主按钮高 ≥56px、做完一件事点击数 ≤3。完整的界面硬性规则见 [docs/06-界面规范](docs/06-界面规范.md)。
...
| **照片转成文字文档** | OCR + 版面重建，输出**保留原有排版**的 Word：文字放在按原位置摆的文本框里，能选中、复制、改错字 |
| **证件印一张纸** | 身份证正反面、户口本两页拼到一张 A4，**按实物尺寸打**（85.6×54mm），尺寸不保真就明确说出来 |
| **微信收到的文件自动出现** | 监控微信接收目录和 `C:\打印\待打印`，新文件直接显示在首页，不用去文件夹里翻 |

**刻意没有「打印文档」这个入口**：原件是 Word/Excel/PDF 时直接在 Office 里改好再打就行，工具在这上面帮不了忙（还多一道转换）。需要打印时都从具体的活儿进去，最后统一落到「打印预览」页 —— 在那里选打印机、份数、纸张。

<table>
<tr>
<td width="50%"><img src="docs/images/照片转文字.png" alt="照片转文字：识别结果可直接编辑，另存为 Word"></td>
<td width="50%"><img src="docs/images/打印预览.png" alt="打印预览：选打印机、份数、纸张，预览画的是整张纸"></td>
</tr>
<tr>
<td align="center">识别结果可以直接改，置信度低的行标红；另存为的 Word 保留原有排版</td>
<td align="center">预览画的是<b>整张纸</b>：内容摆在哪、有没有被缩、四周打不到的边在哪</td>
</tr>
</table>
...
src/shop_print/
  core/     纯逻辑，不 import 任何 Qt —— 第二阶段的服务端 / 店铺 Agent 要原样复用
    ...
    ocr.py        RapidOCR + 版面重建 → docx / txt（可保留原有排版：文字放文本框里）
  ...
  ui/       PySide6，只做展示与调度，不写业务算法（耗时的活一律 run_async）
  texts.py  所有面向用户的中文文案（含友好错误话术）
  paths.py  所有路径统一在此定义
  self_check.py  --self-check 自检，店铺机上排障用
```

一条贯穿设计：**一切文件先归一化成 PDF，再统一预览、统一打印**。预览器只认一种格式，打印路径只有一条，顾客看到的预览和打出来的纸一定是同一份数据；打印预览画的是整张纸，用的还是真打印那套摆放算式。见 [docs/02-架构与分层](docs/02-架构与分层.md)。
